...
class ImporterController extends Controller
{
    ...
    $mytable = DB::connection('mysql2')->table('mytable2')
    ->where('subcon_id','>=',13591)
    ->where('subcon_id','<=',13824)
    ->get();
    ...
    foreach($mytable as $adjust){
        ...
        if($mytable){
            ...
            // ->groupBy('client_id')->first(); 
                ...
                $timesheet = app('App\Http\Controllers\Api\Timesheet\TimesheetController')->getTimeRecords($adjust->leads_id, $mytable->userid,'2023-01-01','2023-01-15');
               ...
               if($timesheet){
                ...
                foreach($timesheet as $index => $t_total){
                 ...
                 $update = DB::connection('mysql2')->table('mytable2')->where('subcon_id', $adjust->subcon_id)->where('leads_id', $adjust->leads_id)->update( [ 'total_log_hours' => number_format($actual_hours_worked, 2), 'total_adjusted_hours' =>  number_format($tot_adj, 2)] );
